...
const NUM_SHAPES = 20; // Adjust for performance vs density
const NUM_PARTICLES_MAX = 100; // Max particles at any time
...
class Shape {
  constructor(canvasWidth, canvasHeight) {
    ...
    this.color = SUBTLE_DARK_THEME_COLORS[Math.floor(Math.random() * SUBTLE_DARK_THEME_COLORS.length)];
  ...
  draw(ctx) {
    ...
    ctx.globalAlpha = 0.7; // Make shapes semi-transparent
    ...
    switch (this.type) {
      ...
      case 'square':
        ...
        break;
      case 'triangle':
        ctx.moveTo(0, -currentSize / 1.5); // Adjusted for better visual balance
        ctx.lineTo(currentSize / 1.5, currentSize / 3);
        ctx.lineTo(-currentSize / 1.5, currentSize / 3);
        ctx.closePath();
        break;
      case 'rectangle':
        ctx.rect(-currentSize * 0.75, -currentSize / 2, currentSize * 1.5, currentSize);
        break;
    }
    ctx.fill();
    ctx.restore();
...
const AnimatedBackground = () => {
  ...
  const drawRadialGradient = useCallback((ctx, width, height) => {
    ...
    const outerRadius = Math.max(width, height) * 0.7; // Ensure gradient covers well

    const gradient = ctx.createRadialGradient(centerX, centerY, 0, centerX, centerY, outerRadius);
    gradient.addColorStop(0, 'rgba(45, 55, 75, 0.1)'); // Center, subtle
    gradient.addColorStop(0.3, 'rgba(30, 40, 60, 0.3)');
    gradient.addColorStop(0.6, 'rgba(20, 30, 50, 0.5)');
    gradient.addColorStop(1, 'rgba(10, 15, 30, 0.7)');   // Edges, darker
  ...
  const animate = useCallback(() => {
    ...
    // Occasionally add new particles (e.g., from shape centers or randomly)
    if (particlesRef.current.length < NUM_PARTICLES_MAX && Math.random() < 0.2) { // Add particles less frequently
        const randomShape = shapesRef.current[Math.floor(Math.random() * shapesRef.current.length)];
        if (randomShape) {
            particlesRef.current.push(new Particle(randomShape.x, randomShape.y, randomShape.color));
        }
    }
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      // Re-initialize canvas and shapes for new dimensions
      initCanvas();
    };
